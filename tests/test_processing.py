...
def test_filter_by_state(value: list, expected: list) -> None:
    assert processing.filter_by_state(value) == expected


# Тестирование функции sort_by_date из модуля src/processing.py
@pytest.mark.parametrize(
    "value, expected",
    [
        (
...
def test_sort_by_date(value: list, expected: list) -> None:
    assert processing.sort_by_date(value) == expected
